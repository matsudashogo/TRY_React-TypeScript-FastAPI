...
import TodoTitleInput from '../parts/TodoTitleInput'
...
const Top: React.FC = () => {
    ...
    const editSaveTodo = (id: number, newTitle: string) => {
        const newTodos = todos.map((todo) => {
            if (todo.id === id) {
                return { ...todo, title: newTitle, updated_at: new Date().toISOString().slice(0, 19).replace('T', ' '), is_edit: !todo.is_edit }
            }
            return todo
        })
                            ...
                            <tr key={todo.id} className="hover:bg-gray-50 transition">
                                ...
                                <td className="px-6 py-4 text-center space-x-2">
                                    <Button
                                        ...
                                        event={() => todo.is_edit ? editSaveTodo(todo.id, todo.title) : toggleEdit(todo.id)}
                                    />
                                    <Button
